...
'''def insert_a_record():
    ...
    worksheet1= wb.sheet_by_index(option-1)
    room_no = input("Enter room number in the form \'A2-22\' \n")
    room_no_split= room_no.split("-")
    name_of_sheet = option-1
    print(" ")
    if (option >= 2 and option <= 10):
        if int(room_no_split[1]) in single_rooms_in_Ablock:
            for i in range(0,worksheet1.nrows):
                if room_no == worksheet1.cell_value(i,2) and  not worksheet1.cell_value(i,4) in single_room_occupancy:
                    list_details=details_form(details_list)
                    print(list_details)
                    for j in range(3,worksheet1.ncols):
                        if i == 4:
                            pass
                        else:
                           # sheet1 = list_details[i]
                            name_of_sheet.write(i, j, list_details[j])
                    yes_no = input("Entre \"y\" if you want to save the details or else \"N\"  ")
                    if yes_no == 'y':
                        name_of_sheet.write(i, 4, "OCCUPIED")
                        wb.save('excel_sheet.xlsx')
                        print("Details where updated")
                    else:
                        print("Dettails where not updated")

def details_form(details_list):
    name = input("Enter name of the student\n")
    #room_number = str(details_list[0])+'-'+str(details_list[1])
    single_double =input("Enter \"SINGLE\" or \"DOUBLE\"\n ")
    status = input("Enter status either OCCUPIED or not \n")
    roll_no = input("enter rol number of the student\n")
    course = input("enter course of the student\n")
    specialization = input("enter specialization of the student\n")
    department = input("enter department of the student\n")
    stu_mobile_no =input("enter student mobile number of the student\n")
    adhar_no = input("enter adhar number the student\n")
    student_mail_id =  input("enter student mail id of the student\n")
    fath_mobile_no = input("enter father mobile number of the student\n")
    moth_mobile_no =  input("enter mother mobile number of the student\n")
    mess_card_no = input("enter mess card of the student\n")
    Icard =  input("enter ICARd of the student\n")
    time= datetime.date.today()
    remarks = ' '
    details_list.append(name)

    #details_list.append(room_number)
    details_list.append(single_double)
    details_list.append(status)
    details_list.append(roll_no)
    details_list.append(course)
    details_list.append(specialization)
    details_list.append(department)
    details_list.append(stu_mobile_no)
    details_list.append(adhar_no)
    details_list.append(student_mail_id)
    details_list.append(fath_mobile_no)
    details_list.append(moth_mobile_no)
    details_list.append(mess_card_no)
    details_list.append(Icard)
    details_list.append(datetime.date.today())
    details_list.append(time)
    details_list.append(remarks)
'''
